...
namespace starPadSDK.Geom
{
	...
	public struct Pt {
        ...
        public static explicit operator Vector(Pt p) { return new Vector(p.X, p.Y); }
        public static explicit operator Pt(Vector v) { return new Pt(v.X, v.Y); }
        public static explicit operator Pt(Vec p) { return new Pt(p.X, p.Y); }
        public static implicit operator Pt(StylusPoint p) { return new Pt(p.X, p.Y); } // seems a hack to stick this here, but also seems only choice--would have preferred to define it in with Polyline (where StylusPoints are relevant) as an extension, but can't do that with operators (arg).
    ...
    public struct Vec {
        ...
        public static implicit operator Vector(Vec v) { return new Vector(v.X, v.Y); }
        public static implicit operator Vec(Vector v) { return new Vec(v.X, v.Y); }
    ...
    public struct Mat {
        ...
        static public Mat Rotate(Deg d) {
            Mat s = new RotateTransform(d.D).Value;
            return s;
        }
        static public Mat Rotate(Rad r, Pt center) {
            Mat s = new RotateTransform(r*180/Math.PI,center.X,center.Y).Value;
            return s;
        }
        ...
        public static implicit operator Matrix(Mat m) {
            return new Matrix(m._e00, m._e10, m._e01, m._e11, m._e02, m._e12);
        }
        public static implicit operator Mat(Matrix m) {
            return new Mat(m);
        }
        public static Mat operator*(Mat a, Mat b) {
            return (Mat)((Matrix)a * (Matrix)b);
        }
    ...
    public struct LnSeg {
        ...
        /// </summary>
        public Pt Center { get { return (Pt)(((Vec)A)+((Vec)B)); } }
        /// <summary>
        ...
        /// <returns></returns>
        public Pt? Intersection(LnSeg b, double epsilon, bool lines, out double t) {
            t = 1;
            double d = (b.B - b.A).PerpDot(B - A);
            if(Math.Abs(d)>epsilon*epsilon) {
                double AB = (B - A).PerpDot(b.A - A)/d;
                if(lines || (AB>0.0 && AB<1.0)) {
                    double CD = (b.B - b.A).PerpDot(b.A - A)/d;
                    if(lines || (CD>0.0 && CD<1.0)) {
                        t = CD;
                        return b.A + AB*(b.B - b.A);
                    }
                }
            }
            return null;
        ...
        /// <param name="b"></param>
        /// <returns></returns>
        public Pt? Intersection(LnSeg b) { double t;   return Intersection(b, Ln.Epsilon, false, out t); }
        /// <summary>
    ...
    public struct Rct {
		...
		public Pt BottomLeft {
			get { return new Pt(Left,Bottom); } set { _topLeft.X = value.X; _bottomRight.Y = value.Y; }
		}
		public Pt TopRight {
			get { return new Pt(Right,Top); } set {  _bottomRight.X = value.X; _topLeft.Y = value.Y; }
		}
        public double Top {
        ...
        public Rct(Pt topLeft, Pt bottomRight) : this() {
            TopLeft = topLeft;
            BottomRight = bottomRight;
        }
        public Rct(Pt p) : this() {
            TopLeft = p;
            BottomRight = p;
        }
        public Rct(Pt topLeft, Vec size) : this() {
            TopLeft = topLeft;
            BottomRight = topLeft + size;
        }
        public Rct(double left, double top, double right, double bottom) : this() {
        ...
        // both ways cause can't define operators on built-in types.
        public static implicit operator Rect(Rct r) { return new Rect(r.Left, r.Top, r.Width, r.Height); }
        public static implicit operator Rct(Rect r) { return new Rct(r.Location, (Vec)r.Size); } 
