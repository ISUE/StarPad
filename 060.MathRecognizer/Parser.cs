...
namespace starPadSDK.MathRecognizer {
    ...
    public class Line {
        ...
        public bool Uses(Symbol n, bool inside, bool allowSupers) {
            ...
            for (int s = _syms.Count - 1; s >= 0; s--) {
                int absCount = 0;
                for (int x = 0; x <= s; x++)
                    if (_syms[x].Sym.Character == '|')
                        absCount++;
                bool isAbs = _syms[s].Sym.Character == '|';
    ...
    public class Parser : IDisposable {
            ...
            for (int i = 0; i < (rangeCount < 0 ? Ranges.Count : Math.Min(Ranges.Count, rangeCount)); i++)
            {
                ...
                while (okay < 5)
                {
                    try
                    {
                        ...
                        //AG Update
                        Ranges[Ranges.Count - i - 1].Updated = true;
        ...
        private void Parse(Strokes strokes, int inkPixel)
        {
