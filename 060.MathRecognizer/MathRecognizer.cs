...
namespace starPadSDK.MathRecognizer {
    public class MathRecognition : IDisposable {
        ...
        #region Batch Edit Without Sketch Recognition
        ...
        #region Sim Add, Delete, Transform

        public void sim_StrokesAdded(Strokes ss)
        {
            _changeSeq++;
            if(BatchEditing) {
        ...
        public void sim_StrokesDeleting(Strokes stks)
        {
            _changeSeq++;
            if (BatchEditing)
            {
        ...
        public void sim_StrokeTransformed(Stroke s, System.Drawing.Drawing2D.Matrix m)
        {
            _changeSeq++;
            if (BatchEditing)
            {
        ...
        private Expr updateExpr(ref Expr ep0, ArrayExpr ep1, String op)
        {
            ...
            else if (ep0 is CompositeExpr)
            {
                ...
                {
                    if (!(ce.Args[1] is WordSym))
                    { // written power overwrites menu choice
        ...
        private MathExpr.Expr patternEntry(int k, char c, Expr e0, Expr e1)
        {
            ...
            if (e0 is LetterSym && e1 is LetterSym)
            {
                if ((e0 as LetterSym).Subscript != null && (e0 as LetterSym).Subscript is IntegerNumber)
                {
                    ...
                    else if ((e1 as LetterSym).Subscript != null && (e1 as LetterSym).Subscript is LetterSym)
                    {//linear for subscripts
                        Expr head = WellKnownSym.plus;
                        Expr[] args = new Expr[2];
                        args[0] = new LetterSym(c);
        ...
        #region Alternative Menu Change
