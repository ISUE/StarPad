...
namespace MathRecoScaffold {
    public class Evaluator {
        ...
        public class Constant {
            public Expr Name;
            public Expr Value;
            public Constant(Expr name, Expr value) { Name = name; Value = value; }
        }
...
# if false             
        ...
        static public Expr SubstMathVars(IEnumerable<Parser.ParseResult> results, Expr toEval) {
            ...
            try {
                ...
                Expr final = toEval;
                final = SubstSubscript(variables, values, final);
                return final;
            } catch(Exception) {
            }
            return new NullExpr();
        ...
        private static Expr SubstSubscript(List<Expr> variables, List<Expr> values, Expr final) {
            if(final is IntegerNumber) return final;
            if(final is LetterSym && (final as LetterSym).Subscript != null) final = new LetterSym((final as LetterSym).Letter, SubstSubscript(variables, values, (final as LetterSym).Subscript));
            if(final is CompositeExpr) {
                int len = (final as CompositeExpr).Args.Length;
                Expr[] theArg = new Expr[len];
                for(int i = 0; i<len; i++) {
                    theArg[i] = SubstSubscript(variables, values, (final as CompositeExpr).Args[i]);
                }
