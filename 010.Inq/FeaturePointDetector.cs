...
namespace starPadSDK.CharRecognizer
{
    ...
    public class FeaturePointDetector
    {
        ...
        public float InkPixel = (float)starPadSDK.Inq.MSInkCompat.OldStrokeStuff.Scale;
        ...
        public SortedList ClearRecogs(Strokes clearStrokes)
        {
        ...
        public Recognition Recognize(CuspSet cset, bool singleOnly)
        {
            ...
            using (RecognizerContext myRecoContext = new RecognizerContext())
            {
                ...
                RecognitionResult recoResult = myRecoContext.Recognize(out status);
        ...
        bool match_9(CuspSet s, ref int xhgt)
        {
            int maxc, startc;
            if (!match_a9gq_start(s, false, out maxc, out startc))
                return false;
        ...
        bool match_delta(CuspSet s)
        {
            ...
            //if starting point is abve the end point, not a delta
            ...
            {
                straightness = V2D.Straightness(s.pts, s.pts.Length - 1, s.pts.Length - 10);
            }
        ...
        public bool match_rect(CuspSet s)
        {
            if (s.cusps.Length == 5)
            {
                ...
                if (!is90(ang1x) || !is90(ang2x) || !is90(ang3x) || !is90(ang4x))
                    return false;
                if (V2D.Dist(s.pts[0], s.last) / s.bbox.Height > 0.2)
                    return false;
                return true;
            }
            return false;
        ...
        bool match2_x(CuspSet s, CuspSet s1, string letter, string other, ref Strokes stks)
        {
            ...
            if (f_ang > 25 && stemtop.X > inter.X && crossx > 45 && Math.Abs((Math.Abs(stem.angles[stemind]) - Math.PI / 2)) < 0.6)
            {
                ...
                if (top_f_ind < 0 || top_f_ind >= stem.angles.Length)
                    return false; // fix an out of bounds exception
            ...
            if (other != "L")
            {
                if (other[0] == '(' || other == "<")
                {
                    ...
                    if (V2D.Straightness(s1.pts) < 0.25 || angle(s1.last, s1.pts[ind], new PointF(1, 0)) > 35)
                        return false;
                    if (angle(s1.pts[5], s1.pts[0], new PointF(-1, 0)) < 15)
                        return false;
                }
                else
                    return false;
        ...
        bool match2_t(CuspSet s, CuspSet s1, string letter, string other, ref int xhgt, ref string newmatch)
        {
            ...
            if (other.Contains("2") || other.Contains("z") || other.Contains(">") || other.Contains("7"))
            {
                ...
                if (s1.cusps[1].top && s1.distances[rightind] / (s1.dist - s1.cusps[2].dist) > 0.35)
                    return false;
            }
            float[] ints = s1.s.FindIntersections(s.s.Ink.CreateStrokes(new int[] { s.s.Id }));
            float[] cints;
        ...
        bool match2_4(CuspSet s, CuspSet s1, string letter, string other, ref int xhgt)
        {
        ...
        bool match_Delta(CuspSet s, ref string allograph)
        {
        ...
        bool match_8(CuspSet s, ref int xhgt)
        {
            if (s.intersects.Length < 2)
                return false;
        ...
        bool match_7(CuspSet s, ref string allograph)
        {
            int startind = 0;
            if (s.cusps.Length >= 4)
            {
